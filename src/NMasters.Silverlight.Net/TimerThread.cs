...
namespace NMasters.Silverlight.Net
{
    internal static class TimerThread
    {
        ...
        private static Dictionary<object, object> s_QueuesCache = new Dictionary<object, object>();
        ...
        internal static Queue GetOrCreateQueue(int durationMilliseconds)
        {
            ...
            bool exists = s_QueuesCache.ContainsKey(durationMilliseconds);
            ...
            if ((reference == null) || ((queue = (TimerQueue) reference.Target) == null))
            {
                lock (s_NewQueues)
                {
                    bool exists1 = s_QueuesCache.ContainsKey(durationMilliseconds);                    
                    if (exists1)
                        reference = (WeakReference)s_QueuesCache[durationMilliseconds];
                    ...
                    foreach (var entry in s_QueuesCache)
                    {
                ...
                if (Interlocked.CompareExchange(ref s_ThreadState, 1, 1) == 1)
                {
                    ...
                    while (flag)
                    {
                        ...
                        Label_0043:
                            ...
                            int num6 = WaitHandle.WaitAny(s_ThreadEvents, millisecondsTimeout);
                            if (num6 == 0)
                            {
                                flag = false;
                            }
                            else
                            {
                                ...
                                if (s_ThreadReadyEvent.WaitOne(0) && (Interlocked.CompareExchange(ref s_ThreadState, 1, 0) == 0))
                                {
                                    goto Label_0043;
                                }
                                flag = false;
